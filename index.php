...
// 2. ОБНОВЛЕННАЯ ЛОГИКА ДОБАВЛЕНИЯ СМЕНЫ (Ввод через количество часов и минут)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_shift'])) {
    ...
    $input_hours = intval($_POST['worked_hours']);   // Считываем часы
    $input_minutes = intval($_POST['worked_minutes']); // Считываем минуты

    if (!empty($emp_id) && !empty($emp_name) && !empty($start_str) && $rate >= 0 && ($input_hours > 0 || $input_minutes > 0)) {
        $start_timestamp = strtotime($start_str);
        
        // Математически вычисляем общую сумму минут и переводим в часы для базы
        $total_minutes = ($input_hours * 60) + $input_minutes;
        $hours_coef = round($total_minutes / 60, 4);

        // Автоматически рассчитываем точную дату и время окончания смены для базы данных
    ...
    } else { $error = "Заполните все поля формы корректно (отработанное время должно быть больше 0)."; }
}

// 3. СБОР ДАННЫХ ДЛЯ СВОДНОЙ ТАБЛИЦЫ
$work_data = [];
$rates_archive = [];
if (isset($_SESSION['authorized'])) {
    $stmt_hours = $pdo->query("SELECT employee_id, employee_name, MAX(hourly_rate) as current_rate,
...
<head>
    ...
    <style>
        ...
        input { padding: 12px; font-size: 14px; width: 100%; box-sizing: border-box; border-radius: 8px; border: 1px solid #333; background: #2a2a2a; color: white; margin-bottom: 15px; outline: none; }
        ...
        .work-table { width: 100%; border-collapse: collapse; margin-top: 20px; background: #2a2a2a; }
    ...
    <div class="secure-content">
        ...
        <?php if (isset($_SESSION['is_admin']) && $_SESSION['is_admin'] == 1): ?>
            ...
            <form method="POST" class="form-inline" id="shiftForm">
                ...
                <!-- НОВЫЕ ПОЛЯ ВМЕСТО ОКОНЧАНИЯ СМЕНЫ -->
                <div>
                ...
                <button type="submit" name="add_shift">Зписать</button>
            </form>
        <?php endif; ?>

        <!-- СВОДНЫЙ ТАБЕЛЬ -->
        <h3>Сводный табель заработка за текущий период</h3>
        <table class="work-table">
            ...
            <tbody>
                <?php if (!empty($work_data)): ?>
                    ...
                    <?php endforeach; ?>
                <?php else: ?>
                    <tr><td colspan="6" style="text-align: center; color: #aaa;">Данные отсутствуют. Заполните форму выше.</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
